...
script() {
    banner
    echo -e "${R} [${W}-${R}]${C} Setting up Installing Script..."${W}

    if [[ -e "$PREFIX/etc/proot-distro/nethunter.sh" ]]; then
        echo -e "${R} [${W}-${R}]${C} script already installed..."${W}
    else
...
permission() {
    ...
    if [[ -e "$PREFIX/bin/nethunter" ]]; then
        ...
        echo -e "\n${R} [${W}-${R}]${G} Run ${C}nethunter${G} first & then type ${C}bash user.sh (if you want to create user"${W}
        echo -e "\n"
        exit 0
    else
